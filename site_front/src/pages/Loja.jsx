  ...
  const handleCompra = async (nomeProduto, precoProduto) => {
    try {
      ...
      if (resposta.ok) {
        ...
        const mensagem = `Olá, Felipe %0A` +
                         `Gostaria de comprar este Hotwheels!%0A%0A` +
    ...
    <div className="loja-container">
      <header className="loja-header">
        ...
        <nav className="loja-menu">
          <button className={`menu-btn ${categoriaAtiva === 'Hotwheels' ? 'ativo' : ''}`} onClick={() => setCategoriaAtiva('Todos')}>Modelos</button>
          <button className={`menu-btn ${categoriaAtiva === 'Combos' ? 'ativo' : ''}`} onClick={() => setCategoriaAtiva('Combos')}>Combos</button>
          {/* <button className={`menu-btn ${categoriaAtiva === 'Hotwheels' ? 'ativo' : ''}`} onClick={() => setCategoriaAtiva('Hotwheels')}>Hotwheels</button>
      ...
      <footer>
        <p>2026 Hotwheels - Todos os direitos reservados. Desenvolvido por Felipe A.</p>
      </footer>
    </div>
  );
}
